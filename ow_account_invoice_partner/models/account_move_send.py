...
class AccountMoveSend(models.AbstractModel):
    ...
    def _get_default_mail_partner_ids(self, move, mail_template, mail_lang):
        ...
        # Ordering matters: super() ends on ``.filtered('email')``, so a customer
        # without an email is already gone from ``partners`` by now. The invoice
        # contact is therefore resolved from ``move`` directly -- resolving it
        # from super()'s output would leave exactly that customer unswapped, and
        ...
        return (partners | invoice_partners).filtered('email')
...
class AccountMoveSendWizard(models.TransientModel):
    ...
    def _compute_mail_subject_body_partners(self):
        ...
        for wizard in self:
            ...
            if wizard.mail_template_id or not wizard.move_id:
                continue
