# OW MCP Server — native Odoo 19 MCP addon
...
- `X-Odoo-Dbfilter: ^my_db$` header — honored by OCA's
  [`dbfilter_from_header`](https://github.com/OCA/server-tools/tree/19.0/dbfilter_from_header)
...
Input: `{}`
Output: `{"modules": [{"name": "sale", "label": "Sales", "version": "19.0.1.0"}, …], "count": 42}`
...
- Key expired → Odoo 19 requires an expiration date; create a new one.
