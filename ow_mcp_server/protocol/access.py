...
Layers (all must pass):
  ...
  3. Odoo native ACL (check_access) — always enforced.
"""
import logging
import os
import re
...
BLOCKLIST_RE = re.compile(
    ...
    r'ir\.(rule|access|config_parameter|logging|attachment|attachment\.history'
...
def check_model_access(env, model_name, op):
    ...
    if not skip_mcp_layer:
        ...
        if ma.group_ids and not (env.user.all_group_ids & ma.group_ids):
            raise AccessError(
                f'User lacks group membership for MCP access to {model_name!r}.'
            )

    # Odoo-native enforcement, always. Empty recordset → model-level ACL
    # check (Odoo 19 replacement for the deprecated check_access_rights).
    env[model_name].check_access(op)
    return True
