...
class MCPAuditLog(models.Model):
    ...
    def cron_cleanup_old_entries(self):
        ...
        while True:
            ...
            batch.unlink()
            total += n
            # Commit between batches so the cron never holds one huge
            # transaction. Never commit under the test runner: Odoo 19 has
            # no registry.in_test_mode() anymore, and a TransactionCase
            # cursor would really commit — so gate on --test-enable.
