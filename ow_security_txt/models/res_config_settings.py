...
class ResConfigSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        res['ow_security_txt_contact'] = self.env['ir.config_parameter'].sudo(
            ).get_str('ow_security_txt.contact', '')
        return res
    ...
    def _render_ow_security_txt(self):
        ...
        contacts = self._ow_security_txt_parse_contacts(
            icp.get_str('ow_security_txt.contact'))
        if not contacts:
            return False
        months = int(icp.get_str('ow_security_txt.expires_months') or 6)
        ...
        for label, param in OPTIONAL_FIELDS:
            value = (icp.get_str(param) or '').strip()
            if value:
